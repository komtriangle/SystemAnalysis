    ...
    for node in nodes:
        if node not in subordination:
            ...
            if node in submission_dict:
                all_subordinations(
                    subordination, submission_dict[node], submission_dict)
...
def process_edges(edges: []):
    ...
    for key in submission_dict:
        ...
        indirect_subordination(
            subordination, submission_dict[key], submission_dict)
...
def entropy(vertices, complexity):
    items = []
    [items.append([]) for _ in vertices]

    for vertex in vertices:
        for j in range(len(complexity)):
            a = complexity[j].count(vertex)
            items[vertex - 1].append(complexity[j].count(vertex))
    ...
    for j in range(len(vertices)):
        for i in range(len(vertices)):
            if items[j][i] != 0:
                entropy += (items[j][i] / (len(vertices) - 1)) * \
                    math.log(items[j][i] / (len(vertices) - 1), 2)
